...
const collabField = state.StateField.define({
    ...
    fromJSON(json) {
        return new CollabState(json.version, json.unconfirmed.map(update => new LocalUpdate(undefined, state.ChangeSet.fromJSON(update.changes), update.effects, update.clientID)));
    }
});
/**
...
exports.getCollabField = getCollabField;
