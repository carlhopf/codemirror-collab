import { Facet, combineConfig, Annotation, StateField, ChangeSet, StateEffect, Transaction } from '@codemirror/state';
...
const collabField = /*@__PURE__*/StateField.define({
    ...
    fromJSON(json) {
        return new CollabState(json.version, json.unconfirmed.map(update => new LocalUpdate(undefined, ChangeSet.fromJSON(update.changes), update.effects, update.clientID)));
    }
});
/**
...
function receiveUpdates(state, updates) {
    ...
    let own = 0;
    while (own < updates.length && unconfirmed[own] && updates[own].clientID == unconfirmed[own].clientID)
        own++;
    if (own) {
...
function getCollabField() {
    return collabField;
}

export { collab, getClientID, getCollabField, getSyncedVersion, receiveUpdates, sendableUpdates };
